...
module.exports.execute = async(client, message, args,ayar) => {
   ...
   // if(!message.member.roles.cache.array().some(rol => message.guild.roles.cache.get(ayar.staffrole).rawPosition <= rol.rawPosition)) return  message.reply("`Bu komut yetkililere özeldir.`");
    ...
    MemberStats.find({guildID: message.guild.id}).exec((err, data) => {
        ...
        embed.setDescription(`**Sander Server Statistics**s`);
        embed.addField('<a:ses:812647122910183425> Haftalık Ses Sıralama', haftalikSesSiralama);
        embed.addField('<a:gl:750965146481983528> Haftalık Mesaj Sıralama', haftalikChatSiralama);
        embed.addField('<a:ses:812647122910183425> Genel Ses Sıralama', toplamSesSiralama);
        embed.addField('<a:gl:750965146481983528> Genel Mesaj Sıralama', toplamChatSiralama);
